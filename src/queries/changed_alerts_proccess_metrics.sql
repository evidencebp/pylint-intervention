...
drop table if exists general.file_properties_in_between;


create table
general.file_properties_in_between
as
select
cf.repo_name as repo_name
, file
...
cf.commit = ec.commit and cf.repo_name = ec.repo_name
where
date(ec.commit_timestamp) < PARSE_DATE('%d/%m/%Y',  '1/8/2022')
and date(ec.commit_timestamp) >= PARSE_DATE('%d/%m/%Y',  '1/2/2022')
group by
repo_name
, file
;
